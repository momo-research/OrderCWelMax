...
AnyOption::setCommandLongPrefix(char* _prefix)
{
	...
	strcpy_s(long_opt_prefix, strlen(_prefix) + 1, _prefix);
}
...
AnyOption::setValue(const char* option, char* value)
{
	...
	for (int i = 0; i < option_counter; i++) {
		if (strcmp(options[i], option) == 0) {
			...
			strcpy_s(values[optionindex[i]], strlen(value) + 1, value);
			return true;
		}
	}
	return false;
...
AnyOption::setFlagOn(char option)
{
	...
	for (int i = 0; i < optchar_counter; i++) {
		if (optionchars[i] == option) {
			...
			strcpy_s(values[optcharindex[i]], strlen(TRUE_FLAG) + 1, TRUE_FLAG);
			return true;
		}
	}
	return false;
...
AnyOption::readFile(const char* fname)
{
	...
	is.close();
	cout << "config buffer:<" << buffer << ">" << endl;
	return buffer;
}
